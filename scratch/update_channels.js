...
async function main() {
    ...
    for (const channel of channels) {
        if (!channel.compiled_content) continue;
        
        if (channel.compiled_content.includes('class="page"') && channel.compiled_content.includes('class="hero-inner"')) {
            const newHtml = updateStyles(channel.compiled_content);
            
            if (newHtml !== channel.compiled_content) {
                await prisma.channel.update({
                    where: { id: channel.id },
                    data: { compiled_content: newHtml }
                });
                count++;
                console.log(`Updated Channel ID: ${channel.id} - ${channel.name}`);
            }
        }
    }
    
    console.log(`Successfully updated ${count} channels in the database.`);
}
